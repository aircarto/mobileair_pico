// Adapted from pico-examples access_point dhcpserver (MIT License, Damien P. George)
// Modified for MobileAir: base IP .100, pool size 11
...
static int dhcp_socket_sendto(struct udp_pcb **udp, struct netif *nif,
                              const void *buf, size_t len, uint32_t ip, uint16_t port) {
    if (len > 0xffff) {
        len = 0xffff;
    }
...
static void dhcp_server_process(void *arg, struct udp_pcb *upcb, struct pbuf *p,
    ...
    // Copy into local struct (safe with chained pbufs)
...

    {
        ...
        switch (msgtype[2]) {
            case DHCPDISCOVER: {
                ...
                if (yi == DHCPS_MAX_IP) {
                    printf("[dhcp] No free IP addresses\n");
                    goto ignore_request;
                }
                dhcp_msg.yiaddr[3] = DHCPS_BASE_IP + yi;
                opt_write_u8(&opt, DHCP_OPT_MSG_TYPE, DHCPOFFER);
                printf("[dhcp] OFFER 192.168.4.%d\n", DHCPS_BASE_IP + yi);
                break;
            }
            ...
            case DHCPREQUEST: {
                ...
                printf("[dhcp] ACK client: MAC=%02x:%02x:%02x:%02x:%02x:%02x IP=%u.%u.%u.%u\n",
                    dhcp_msg.chaddr[0], dhcp_msg.chaddr[1], dhcp_msg.chaddr[2],
                    dhcp_msg.chaddr[3], dhcp_msg.chaddr[4], dhcp_msg.chaddr[5],
